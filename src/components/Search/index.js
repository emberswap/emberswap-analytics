...
import { useAllPairsInEmberswap, useAllTokensInEmberswap } from '../../contexts/GlobalData'
...
const Wrapper = styled.div`
  ...
  padding: 12px 16px;
  border-radius: 7px;
  border: ${({ theme, small, open }) =>
    small ? (open ? 'none' : '1px solid #40444f') : transparentize(0.4, theme.bg6)};
...
export const Search = ({ small = false }) => {
  let allTokens = useAllTokensInEmberswap()
  const allTokenData = useAllTokenData()

  let allPairs = useAllPairsInEmberswap()
        ...
        <Input
          ...
          placeholder={
            small
              ? 'Search'
              : below410
              ? 'Search Ember Analytics'
              : below470
              ? 'Search Ember Analytics'
              : below700
              ? 'Search pairs and tokens...'
              : 'Search EmberSwap pairs and tokens...'
          }
          value={value}
          onChange={(e) => {
